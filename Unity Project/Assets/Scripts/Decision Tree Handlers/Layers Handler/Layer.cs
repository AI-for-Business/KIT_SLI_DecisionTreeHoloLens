...
public class Layer
{
    ...
    /// <summary>
    /// Checks each node in the Layer if it contains the datapoint with the searched index
    /// </summary>
    /// <param name="dataPointIndex"></param>
    /// <returns>frame with the dp or null if dp not found </returns>
    public GameObject FrameWithDatapoint(int dataPointIndex)
    {
        ArrayList nodes1 = nodes;


        if (nodes.Count == 0)
        {
            nodes1 = prevLayer.nodes;
        }
        //GameObject searchedFrame = null;
        foreach (GameObject frame in nodes1)
        {

            List<DataPointNew> dpsInFrame = frame.GetComponent<FrameHandler>().dataPoints;

            foreach (DataPointNew dp in dpsInFrame)
            {
                if (dp.index.Equals(dataPointIndex))
                {
                    return frame;
                }
            }
    ...
    protected void InfoAboutNewLayer()
    {
        try
        {
            VuforiaImageTargetFound.NewLayerActivatedDT();
        }
