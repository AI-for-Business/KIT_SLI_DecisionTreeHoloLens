...
public class DecisionTreeHandler : MonoBehaviour
{
    ...
    public GameObject DT_FrameWithDataPoint(int dataPointIndex)
    {
        GameObject frameWithDP = null;
        try
        {
            Layer currentLayer = (Layer)DecisionTreeHandler.s_layers[s_layers.Count - 1];
            //current layer after rebuilt is the last layer 
            frameWithDP = currentLayer.FrameWithDatapoint(dataPointIndex);
        ...
        {
            Debug.LogError("DTHandler NullPointer Frame with DP: ");
        }
